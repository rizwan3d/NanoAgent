...
namespace StemCode.Tests.Infrastructure.CustomTools;
...
public sealed class CustomToolTests
{
    ...
    {
        string workspacePath = Path.Combine(Path.GetTempPath(), "StemCode-CustomToolTest");
        FakeProcessRunner processRunner = new(request =>
        {
            ...
            request.EnvironmentVariables.Should().ContainKey("STEMCODE_CUSTOM_TOOL_NAME");
    ...
    public async Task ExecuteAsync_Should_ExposeSandboxMetadataToProcessEnvironment()
    {
        ...
        capturedRequest.EnvironmentVariables!["STEMCODE_SANDBOX_MODE"].Should().Be("read-only");
        capturedRequest.EnvironmentVariables["STEMCODE_SANDBOX_EFFECTIVE_MODE"].Should().Be("read-only");
        capturedRequest.EnvironmentVariables["STEMCODE_SANDBOX_PERMISSIONS"].Should().Be("use_default");
        capturedRequest.EnvironmentVariables["STEMCODE_CUSTOM_TOOL_TRUSTED"].Should().Be("1");
        capturedRequest.EnvironmentVariables["STEMCODE_WORKSPACE_ROOT"].Should().Be(Path.GetFullPath(workspacePath));
    }
    ...
    private static ToolExecutionContext CreateContext(string argumentsJson)
    {
        return CreateContext(
            argumentsJson,
            Path.Combine(Path.GetTempPath(), "StemCode-CustomToolTest"));
    }
    ...
    {
        string root = workspacePath ?? Path.Combine(Path.GetTempPath(), "StemCode-CustomToolTest");
        ...
        string workspacePath)
    {
        ...
        return new ToolExecutionContext(
            ...
            new ReplSessionContext(
                new StemCode.Domain.Models.AgentProviderProfile(
                    StemCode.Domain.Models.ProviderKind.OpenAi,
                    null),
                "gpt-5-mini",
                ["gpt-5-mini"],
