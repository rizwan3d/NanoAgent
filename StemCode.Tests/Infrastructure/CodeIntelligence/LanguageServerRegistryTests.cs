...
namespace StemCode.Tests.Infrastructure.CodeIntelligence;
...
public sealed class LanguageServerRegistryTests : IDisposable
{
    ...
    {
        _tempRoot = Path.Combine(Path.GetTempPath(), $"StemCode-LspRegistry-{Guid.NewGuid():N}");
    ...
    public async Task ResolveAsync_Should_OrderDetectedServersByPriority()
    {
        WriteFile(
            ...
            """);
        WriteFile(Path.Combine(_workspaceRoot, ".stemcode", "bin", "low.cmd"), "@echo off");
        WriteFile(Path.Combine(_workspaceRoot, ".stemcode", "bin", "high.cmd"), "@echo off");
            ...
            """
            {
              "languageServers": {
                "custom-ruby": {
                  "language": "Ruby",
                  "name": "Custom Ruby",
                  "command": ".stemcode/bin/ruby-lsp.cmd",
                  "languageId": "ruby",
                  "fileExtensions": [".rb"],
                  "priority": 200,
