...
namespace StemCode.Tests.Infrastructure.Git;

public sealed class GitAutoCommitServiceTests
{
    [Fact]
    public async Task TryAutoCommitAsync_Should_AddStemCodeCoAuthorTrailerToAutoCommit()
    {
        string workspacePath = Path.Combine(
            Path.GetTempPath(),
            "stemcode-autocommit-service-tests-" + Guid.NewGuid().ToString("N"));
