...
public sealed class WorkspaceSkillServiceTests : IDisposable
{
    ...
    public WorkspaceSkillServiceTests()
    {
        _workspaceRoot = Path.Combine(
            Path.GetTempPath(),
            $"StemCode-Skills-{Guid.NewGuid():N}");
    ...
    {
        string skillDirectory = Path.Combine(_workspaceRoot, ".stemcode", "skills", "dotnet");
    ...
    {
        string skillDirectory = Path.Combine(_workspaceRoot, ".stemcode", "skills", "frontend");
        Directory.CreateDirectory(skillDirectory);
        await File.WriteAllTextAsync(
            Path.Combine(skillDirectory, "SKILL.md"),
            """
        ...
        result.Path.Should().Be(".stemcode/skills/frontend/SKILL.md");
    ...
    {
        string skillDirectory = Path.Combine(_workspaceRoot, ".stemcode", "skills");
        Directory.CreateDirectory(skillDirectory);
        await File.WriteAllTextAsync(
        ...
        skills[0].Path.Should().Be(".stemcode/skills/code-review.md");
    }
