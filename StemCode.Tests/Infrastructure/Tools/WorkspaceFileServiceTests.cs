...
public sealed class WorkspaceFileServiceTests : IDisposable
{
    ...
    public WorkspaceFileServiceTests()
    {
        _workspaceRoot = Path.Combine(
            Path.GetTempPath(),
            $"StemCode-Workspace-{Guid.NewGuid():N}");
    ...
    [Fact]
    public async Task ListDirectoryAsync_Should_ExcludeStemCodeIgnoredPaths()
    {
        await WriteStemCodeIgnoreAsync(
            """
            *.secret
        ...
        result.Entries.Select(static entry => entry.Path)
            .Should()
            .BeEquivalentTo([".stemcode", ".stemcode/.stemcodeignore", "keep.secret", "public.txt"]);
    }

    [Fact]
    public async Task ReadFileAsync_Should_DenyStemCodeIgnoredPath()
    {
        await WriteStemCodeIgnoreAsync("*.secret");
        await File.WriteAllTextAsync(Path.Combine(_workspaceRoot, "token.secret"), "hidden", CancellationToken.None);
        WorkspaceFileService sut = CreateSut();
    ...
    [Fact]
    public async Task SearchTextAsync_Should_ExcludeStemCodeIgnoredFiles()
    {
        await WriteStemCodeIgnoreAsync(
            """
            secrets/
    ...
    [Fact]
    public async Task WriteFileAsync_Should_DenyStemCodeIgnoredPath()
    {
        await WriteStemCodeIgnoreAsync("secrets/");
        WorkspaceFileService sut = CreateSut();
    ...
    [Fact]
    public async Task ApplyPatchAsync_Should_DenyStemCodeIgnoredPath()
    {
        await WriteStemCodeIgnoreAsync("*.secret");
        WorkspaceFileService sut = CreateSut();
    ...
    private async Task WriteStemCodeIgnoreAsync(string content)
    {
    ...
    private static string CreateOutsideDirectory()
    {
        string outsideRoot = Path.Combine(
            Path.GetTempPath(),
            $"StemCode-Outside-{Guid.NewGuid():N}");
