...
public sealed class StemCodeMcpTomlParserTests : IDisposable
{
    private readonly string _tempDirectory;

    public StemCodeMcpTomlParserTests()
    {
        _tempDirectory = Path.Combine(Path.GetTempPath(), "stemcode-mcp-tests-" + Guid.NewGuid().ToString("N"));
    ...
    public void Parse_Should_ReadStdioServer()
    {
        ...
        McpServerConfiguration server = StemCodeMcpTomlParser.Parse(configPath)
            .Should()
            .ContainSingle()
            .Subject;
