...
namespace StemCode.Tests.Infrastructure.Mcp;

public sealed class StemCodeMcpConfigLoaderTests : IDisposable
{
    private readonly string _tempRoot;
    private readonly string _userConfigPath;
    private readonly string _workspaceRoot;

    public StemCodeMcpConfigLoaderTests()
    {
        _tempRoot = Path.Combine(
            Path.GetTempPath(),
            $"StemCode-McpLoader-{Guid.NewGuid():N}");
        _workspaceRoot = Path.Combine(_tempRoot, "workspace");
        _userConfigPath = Path.Combine(_tempRoot, "appdata", "StemCode", "agent-profile.json");

        Directory.CreateDirectory(_workspaceRoot);
    }

    [Fact]
    public void Load_Should_ReadStemCodeUserAndWorkspaceConfigs()
    {
        WriteConfig(
            _userConfigPath,
            """
        ...
        WriteConfig(
            ...
            """);
        StemCodeMcpConfigLoader sut = CreateLoader();
    ...
    {
        WriteConfig(
            Path.Combine(_workspaceRoot, ".stemcode", "config.toml"),
            """
            [mcp_servers.legacy]
            command = "npx"
            """);
        WriteConfig(
            Path.Combine(_tempRoot, "appdata", "StemCode", "mcp.toml"),
            """
    ...
    public void Load_Should_MergeSessionScopedAcpServersAfterUserAndWorkspaceConfigs()
    {
        ...
        StemCodeMcpConfigLoader sut = CreateLoader([shared, editor]);
    ...
    public void Load_Should_DefaultBlankSourceToAcpSession_AndCopyAssignedFields()
    {
        ...
        sessionServer.HttpHeaders["X-Editor"] = "stemcode";
        ...
        StemCodeMcpConfigLoader sut = CreateLoader([sessionServer]);
        ...
        result.HttpHeaders.Should().Contain("X-Editor", "stemcode");
    ...
    {
        bool result = StemCodeMcpConfigLoader.IsAcpSource(source);
    ...
    private StemCodeMcpConfigLoader CreateLoader(
        IReadOnlyList<BackendMcpServerConfiguration>? sessionMcpServers = null)
    {
        return new StemCodeMcpConfigLoader(
