...
namespace StemCode.Tests.Application.Permissions;
...
public sealed class ToolPermissionEvaluatorTests : IDisposable
{
    ...
    public ToolPermissionEvaluatorTests()
    {
        _workspaceRoot = Path.Combine(
            Path.GetTempPath(),
            $"StemCode-Permissions-{Guid.NewGuid():N}");
    ...
    public void Evaluate_Should_RequireMemoryApprovalForFileWritesUnderRepoMemory()
    {
        ...
        PermissionEvaluationResult result = sut.Evaluate(
            ...
            new PermissionEvaluationContext(CreateContext(
                """
                {
                  "path": ".stemcode/memory/architecture.md",
                  "content": "# Architecture"
                }
                """,
        ...
        result.Decision.Should().Be(PermissionEvaluationDecision.RequiresApproval);
        result.ReasonCode.Should().Be("memory_write_approval_required");
        result.Request!.Subjects.Should().Contain(".stemcode/memory/architecture.md");
        result.Request.ToolTags.Should().Contain("memory_write");
    }
    ...
    public void Evaluate_Should_DenyDeletingLastProjectFile_When_AutoApproveAllToolsIsEnabled()
    {
        ...
        Directory.CreateDirectory(Path.Combine(_workspaceRoot, ".stemcode"));
        File.WriteAllText(Path.Combine(_workspaceRoot, ".git", "HEAD"), "ref: refs/heads/main");
        File.WriteAllText(Path.Combine(_workspaceRoot, ".stemcode", "state.json"), "{}");
    ...
    private sealed class StubWorkspaceRootProvider : global::StemCode.Application.Abstractions.IWorkspaceRootProvider
    {
        private readonly string _workspaceRoot;
