...
public sealed class CustomSlashCommandServiceTests : IDisposable
{
    ...
    public CustomSlashCommandServiceTests()
    {
        _tempDirectory = Path.Combine(
            Path.GetTempPath(),
            "stemcode-custom-commands-" + Guid.NewGuid().ToString("N"));
        _workspaceRoot = Path.Combine(_tempDirectory, "workspace");
        _userCommandsDirectory = Path.Combine(_tempDirectory, "user", ".stemcode", "commands");
    ...
    [Fact]
    public void TryExpand_Should_IgnoreBuiltInSkillCommandName()
    {
        WriteWorkspaceCommand(
            "skill.md",
            """
            This should not replace built-in skill management.
            """);
    ...
    {
        WriteCommand(Path.Combine(_workspaceRoot, ".stemcode", "commands"), relativePath, content);
    }
