...
public sealed class SessionCommandSupportTests : IDisposable
{
    ...
    {
        _tempRoot = Path.Combine(Path.GetTempPath(), "stemcode-session-support-tests-" + Guid.NewGuid().ToString("N"));
    ...
    public void CreateDefaultExportPath_ShouldSanitizeTitleAndExtension()
    {
        ...
        Path.GetFileName(path).Should().Be($"stemcode-my-session-v12-{session.SectionId[..8]}.json");
    }
