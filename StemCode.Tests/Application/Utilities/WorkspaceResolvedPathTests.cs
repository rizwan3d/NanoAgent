...
public sealed class WorkspaceResolvedPathTests : IDisposable
{
    ...
    {
        _workspaceRoot = Path.Combine(Path.GetTempPath(), $"StemCode-Resolved-{Guid.NewGuid():N}");
        _outsideRoot = Path.Combine(Path.GetTempPath(), $"StemCode-Resolved-Outside-{Guid.NewGuid():N}");
