...
namespace StemCode.Tests.Application.Formatting;
...
public sealed class ToolOutputFormatterTests
{
    ...
    public void DescribeCall_ShouldSummarizeKnownToolArguments()
    {
        ...
        ConversationToolCall webCall = new(
            ...
            "web_search",
            """
            { "search_query": [{ "q": "stemcode coverage" }] }
            """);
        ...
        _sut.DescribeCall(webCall).Should().Be("web search: \"stemcode coverage\"");
    ...
    public void FormatResults_ShouldSummarizeWebSearchSearchesAndWarnings()
    {
            ...
            CreateResult(
                ...
                """
                {
                  "SearchQuery": [
                    {
                      "Query": "stemcode",
                      "Content": "Title: StemCode\nURL: https://example.test/stemcode",
                      "Results": [
                        {
                          "Title": "StemCode",
                          "Url": "https://example.test/stemcode"
        ...
        messages[0].Should().Contain("search \"stemcode\": 1 result");
        messages[0].Should().Contain("StemCode - https://example.test/stemcode");
