...
namespace StemCode.VS.ToolWindows
{
    ...
    /// File links navigate via the "stemcodefile" scheme; the host handles Hyperlink.RequestNavigate.
    ...
    public sealed class MarkdownPresenter : ContentControl
    {
        ...
        public static Uri FileUri(string path, int line = 0)
            => new("stemcodefile://open/?p=" + Uri.EscapeDataString(path ?? string.Empty) + "&l=" + line);
