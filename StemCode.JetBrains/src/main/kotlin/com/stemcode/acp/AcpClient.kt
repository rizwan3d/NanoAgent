...
/**
 * Client for the StemCode ACP (Agent Client Protocol).
 *
 * Communicates with `stemcode --acp` via JSON-RPC 2.0 over stdin/stdout.
 */
class AcpClient(
    private val stemcodePath: String = "stemcode",
    ...
    /**
     * Start the StemCode process and perform the ACP initialize handshake.
     */
            ...
            Thread {
                try {
                    errorReader.lines().forEach { line ->
                        if (line.isNotBlank()) {
                            logger.warn("StemCode stderr: $line")
                        }
                    }
                } catch (_: IOException) {
                }
