...
/**
 * Editor integration actions for StemCode.
 *
...
abstract class EditorActionBase : AnAction(), DumbAware {
    ...
    protected fun getChatPanel(project: Project): ChatPanel? {
        val toolWindow = ToolWindowManager.getInstance(project).getToolWindow("StemCode") ?: return null
    ...
    override fun actionPerformed(e: AnActionEvent) {
        ...
        val (fileName, path, text) = getSelectedText(e) ?: run {
            com.intellij.openapi.ui.Messages.showWarningDialog(
                project, "Select code or text before sending it to StemCode.", "No Selection"
            )
            return
        }
...
class ExplainSelectionAction : EditorActionBase() {
    override fun actionPerformed(e: AnActionEvent) {
        ...
        val (fileName, _, text) = getSelectedText(e) ?: run {
            com.intellij.openapi.ui.Messages.showWarningDialog(
                project, "Select code or text before asking StemCode to explain it.", "No Selection"
            )
            return
        }
...
class SendCurrentFileAction : EditorActionBase() {
    override fun actionPerformed(e: AnActionEvent) {
        ...
        val (fileName, text) = getCurrentFileText(e) ?: run {
            com.intellij.openapi.ui.Messages.showWarningDialog(
                project, "Open a file before sending it to StemCode.", "No File"
            )
            return
        }
...
class ReviewCurrentFileAction : EditorActionBase() {
    override fun actionPerformed(e: AnActionEvent) {
        ...
        val (fileName, text) = getCurrentFileText(e) ?: run {
            com.intellij.openapi.ui.Messages.showWarningDialog(
                project, "Open a file before asking StemCode to review it.", "No File"
            )
            return
        }
    ...
    override fun actionPerformed(e: AnActionEvent) {
        ...
        if (diff == null) {
            com.intellij.openapi.ui.Messages.showInfoMessage(
                project, "No git diff found for StemCode to review.", "No Git Diff"
            )
            return
        }
...
class ApplySuggestedChangesAction : EditorActionBase() {
    override fun actionPerformed(e: AnActionEvent) {
        val project = e.project ?: return
        val prompt = "Apply the suggested changes from the previous StemCode response."
        getChatPanel(project)?.let { /* send via session manager */ }
    }
