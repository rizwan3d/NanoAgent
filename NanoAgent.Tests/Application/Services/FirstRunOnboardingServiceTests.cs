...
public sealed class FirstRunOnboardingServiceTests
{
    ...
    [Fact]
    public async Task EnsureOnboardedAsync_Should_SaveGeminiCliConfiguration_When_AccountProviderIsSelected()
    {
        AgentProviderProfile profile = new(ProviderKind.GeminiCli, null);

        Mock<ISelectionPrompt> selectionPrompt = new(MockBehavior.Strict);
        SetupProviderSelection(selectionPrompt, OnboardingProviderChoice.GeminiCli);
        ...
        statusMessageWriter
            .Setup(writer => writer.ShowSuccessAsync(
                "Onboarding complete. Provider: Gemini CLI.",
                It.IsAny<CancellationToken>()))
            .Returns(Task.CompletedTask);
        ...
        configurationStore
            .Setup(store => store.SaveAsync(
                new AgentConfiguration(profile, null, null, "Gemini CLI"),
                It.IsAny<CancellationToken>()))
            .Returns(Task.CompletedTask);

        Mock<IApiKeySecretStore> secretStore = new(MockBehavior.Strict);
        secretStore.Setup(store => store.LoadAsync(It.IsAny<CancellationToken>())).ReturnsAsync((string?)null);
        secretStore.Setup(store => store.SaveAsync("gemini-cli-credential-json", It.IsAny<CancellationToken>())).Returns(Task.CompletedTask);
        secretStore.Setup(store => store.SaveAsync(It.IsAny<string?>(), "gemini-cli-credential-json", It.IsAny<CancellationToken>())).Returns(Task.CompletedTask);

        Mock<IAgentProviderProfileFactory> profileFactory = new(MockBehavior.Strict);
        profileFactory.Setup(factory => factory.CreateGeminiCli()).Returns(profile);

        Mock<IGeminiCliAuthenticator> authenticator = new(MockBehavior.Strict);
        authenticator
            .Setup(service => service.AuthenticateAsync(It.IsAny<CancellationToken>()))
            .ReturnsAsync("gemini-cli-credential-json");
        ...
        FirstRunOnboardingService sut = CreateSut(
            ...
            secretStore.Object,
            profileFactory.Object,
            geminiCliAuthenticator: authenticator.Object);
        ...
        result.Should().Be(new OnboardingResult(
            profile,
            true,
            ActiveProviderName: "Gemini CLI"));
        profileFactory.Verify(factory => factory.CreateGeminiCli(), Times.Once);
        authenticator.Verify(service => service.AuthenticateAsync(It.IsAny<CancellationToken>()), Times.Once);
        textPrompt.VerifyNoOtherCalls();
        secretPrompt.VerifyNoOtherCalls();
        inputValidator.VerifyNoOtherCalls();
