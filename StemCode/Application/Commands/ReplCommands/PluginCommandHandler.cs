...
internal sealed class PluginCommandHandler : IReplCommandHandler
{
    ...
    public string Description => "Manage data-only skill marketplaces and installs.";

    public string Usage => "/skill [marketplace add <owner/repo> [--ref <ref>] [--alias <alias>]|marketplace remove <alias>|browse <marketplaceAlias>|install <skillId>@<marketplaceAlias> [--force]|list|uninstall <skillId>]";
        ...
        try
        {
            return context.Arguments[0].Trim().ToLowerInvariant() switch
            {
                ...
                _ => ReplCommandResult.Continue(
                    $"Unknown skill action '{context.Arguments[0]}'.\n{BuildHelpText()}",
        ...
        CancellationToken cancellationToken)
    {
        if (context.Arguments.Count < 2)
        {
            return ReplCommandResult.Continue(
                "Usage: /skill marketplace <add <owner>/<repo> [--ref <ref>] [--alias <alias>]|remove <alias>>",
                ReplFeedbackKind.Error);
        }
        ...
        CancellationToken cancellationToken)
    {
        ...
        {
            return ReplCommandResult.Continue(
                "Usage: /skill marketplace add <owner>/<repo> [--ref <ref>] [--alias <alias>]",
                ReplFeedbackKind.Error);
        }
        ...
        return ReplCommandResult.Continue(
            $"Added skill marketplace '{result.Alias}' -> {result.Entry.Repository}@{result.Entry.Ref}.",
        ...
        CancellationToken cancellationToken)
    {
        ...
        {
            return ReplCommandResult.Continue(
                "Usage: /skill marketplace remove <alias>",
                ReplFeedbackKind.Error);
        }
        ...
        return ReplCommandResult.Continue(
            $"Removed skill marketplace '{result.Alias}' -> {result.Removed.Repository}@{result.Removed.Ref}.",
            ReplFeedbackKind.Info);
    }
        ...
        CancellationToken cancellationToken)
    {
        ...
        {
            return ReplCommandResult.Continue(
                "Usage: /skill browse <marketplaceAlias>",
                ReplFeedbackKind.Error);
        }
        ...
        builder.AppendLine(
            $"Skills available in '{result.Alias}' ({result.Marketplace.Repository}@{result.Marketplace.Ref}):");
        ...
        CancellationToken cancellationToken)
    {
        ...
        {
            return ReplCommandResult.Continue(
                "Usage: /skill install <skillId>@<marketplaceAlias> [--force]",
                ReplFeedbackKind.Error);
        }
        ...
        StringBuilder builder = new();
        builder.AppendLine(
            $"Installed skill '{result.InstalledPlugin.PluginId}' from '{result.InstalledPlugin.MarketplaceAlias}' ({result.InstalledPlugin.Repository}@{result.InstalledPlugin.Ref}).");
        ...
        CancellationToken cancellationToken)
    {
        ...
        {
            return ReplCommandResult.Continue("Usage: /skill list", ReplFeedbackKind.Error);
        }
        ...
        StringBuilder builder = new();
        builder.AppendLine("Skill marketplaces:");
        ...
        builder.AppendLine();
        builder.AppendLine("Installed skills:");
        ...
        CancellationToken cancellationToken)
    {
        ...
        StringBuilder builder = new();
        builder.AppendLine($"Uninstalled skill '{result.PluginId}'.");
    ...
    private static string BuildHelpText()
    {
        return string.Join(
            ...
            [
                "Skill commands:",
                "/skill marketplace add <owner>/<repo> [--ref <ref>] [--alias <alias>]",
                "/skill marketplace remove <alias>",
                "/skill browse <marketplaceAlias>",
                "/skill install <skillId>@<marketplaceAlias> [--force]",
                "/skill list",
