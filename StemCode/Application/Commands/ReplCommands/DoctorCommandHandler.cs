...
internal sealed class DoctorCommandHandler : IReplCommandHandler
{
    ...
    public string Description => "Show comprehensive system diagnostics for StemCode.";
        ...
        CancellationToken cancellationToken)
    {
        ...
        report.AppendLine("╔═══════════════════════════════════════════════");
        report.AppendLine("║  StemCode Doctor - System Diagnostics");
        report.AppendLine("╚═══════════════════════════════════════════════");
        report.AppendLine();
    ...
    private static void AppendWorkspaceInfo(StringBuilder report, ReplCommandContext context)
    {
        ...
        string stemcodeDir = Path.Combine(context.Session.WorkspacePath, ".stemcode");
        report.AppendLine($"  .stemcode:         {Directory.Exists(stemcodeDir)}");

        string profileFile = Path.Combine(stemcodeDir, "agent-profile.json");
        report.AppendLine($"  agent-profile.json: {File.Exists(profileFile)}");

        string memDir = Path.Combine(stemcodeDir, "memory");
        report.AppendLine($"  .stemcode/memory:  {Directory.Exists(memDir)}");

        string skillsDir = Path.Combine(stemcodeDir, "skills");
        report.AppendLine($"  .stemcode/skills:  {Directory.Exists(skillsDir)}");
        report.AppendLine();
    }
    ...
    private static bool IsRunningInWindowsSandbox()
    {
        try
        {
            string? sandboxFlag = Environment.GetEnvironmentVariable("STEMCODE_SANDBOX");
