...
internal static class BuiltInAgentProfiles
{
    ...
    private static readonly IReadOnlySet<string> BuildTools = new HashSet<string>(
        [
            ...
            AgentToolNames.SearchAndReplace,
    ...
    public static IAgentProfile Build { get; } = new BuiltInAgentProfile(
        ...
        Repo memory from .stemcode/memory/*.md is retrieved as reviewable team context when present. Use repo_memory for durable architecture, convention, decision, known-issue, and test-strategy notes; writes require approval and should stay inspectable, diffable, and version-controlled.
        Lesson memory is off by default and can be enabled per workspace with /lessons on. When enabled, relevant lessons from .stemcode/memory/lessons.jsonl are injected automatically into prompts, and automatic tool-failure observation can create or refine lessons about repeated mistakes and fixes.
    ...
    public static IAgentProfile General { get; } = new BuiltInAgentProfile(
        ...
        You are a subagent invoked by a primary StemCode profile for a focused delegated task.
