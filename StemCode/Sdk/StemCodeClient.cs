...
/// <summary>
/// Embeddable, headless entry point for driving a StemCode coding agent from
...
/// </code>
/// </example>
public sealed class StemCodeClient : IAsyncDisposable
{
    private readonly StemCodeBackend _backend;
    private readonly HeadlessUiBridge _uiBridge;
    private bool _initialized;

    internal StemCodeClient(
        StemCodeBackend backend,
    ...
    /// <summary>Creates a new fluent builder for configuring a <see cref="StemCodeClient"/>.</summary>
    public static StemCodeClientBuilder CreateBuilder()
    {
        return new StemCodeClientBuilder();
    }
    ...
    /// </summary>
    public async Task<StemCodeSession> InitializeAsync(CancellationToken cancellationToken = default)
    {
        BackendSessionInfo info = await _backend.InitializeAsync(_uiBridge, cancellationToken)
            .ConfigureAwait(false);
        _initialized = true;
        return StemCodeSession.FromBackend(info);
    }
    ...
    private void EnsureInitialized()
    {
        if (!_initialized)
        {
            throw new InvalidOperationException(
                "The StemCode client has not been initialized. Call InitializeAsync(...) first.");
        }
    }
