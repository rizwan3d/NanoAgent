...
internal sealed class PluginService : IPluginService
{
    ...
    private const string ManifestFileName = "stemcode-plugin.json";
    private const string MarketplaceIndexFileName = "stemcode-marketplace.json";
    private const string PluginsDirectoryRelativePath = ".stemcode/skills";
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (!marketplaces.TryGetValue(normalizedMarketplaceAlias, out PluginMarketplaceEntry? marketplace))
        {
            throw new InvalidOperationException(
                $"Skill marketplace '{normalizedMarketplaceAlias}' is not configured.");
        }
        ...
        {
            throw new InvalidOperationException(
                $"Skill '{normalizedPluginId}' did not expose any installable files.");
        }
        ...
        foreach (PluginManifestFile manifestFile in manifestFiles)
        {
            ...
            {
                throw new InvalidOperationException(
                    $"Skill target '{normalizedTarget}' is a directory, not a file.");
            }

            if (!force && File.Exists(fullTargetPath))
            {
                throw new InvalidOperationException(
                    $"Skill target '{normalizedTarget}' already exists. Re-run with --force to overwrite it.");
            }
        ...
        CancellationToken cancellationToken)
    {
        ...
        {
            throw new InvalidOperationException(
                $"Skill marketplace '{normalizedAlias}' is not configured.");
        }
        ...
        CancellationToken cancellationToken)
    {
        ...
        {
            throw new InvalidOperationException($"Skill '{normalizedPluginId}' is not installed.");
        }
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (await TryFetchTextFileAsync(marketplace, skillSource, cancellationToken) is not null)
        {
            ...
            {
                Source = skillSource,
                Target = $".stemcode/skills/{pluginId}/SKILL.md",
                Kind = "skill"
            });
        }
        ...
        if (await TryFetchTextFileAsync(marketplace, agentsSource, cancellationToken) is not null)
        {
            ...
            {
                Source = agentsSource,
                Target = $".stemcode/skills/{pluginId}/AGENTS.md",
                Kind = "instructions"
            });
        }
        ...
        CancellationToken cancellationToken)
    {
        return await TryFetchTextFileAsync(marketplace, sourcePath, cancellationToken)
            ?? throw new InvalidOperationException(
                $"Skill source file '{sourcePath}' was not found in {marketplace.Repository}@{NormalizeOptional(marketplace.Ref) ?? DefaultRef}.");
    }
    ...
    private static string NormalizePluginId(string pluginId)
    {
        ...
        return IsSimpleIdentifier(normalized)
            ? normalized
            : throw new InvalidOperationException(
                $"Skill id '{pluginId}' is invalid. Use letters, digits, '.', '-', or '_'.");
    }
    ...
    private static string NormalizeManifestSource(string source)
    {
        ...
        {
            throw new InvalidOperationException("Skill sources cannot target .git paths.");
        }
        ...
        string target)
    {
        ...
        {
            throw new InvalidOperationException(
                $"Skill target '{target}' is not allowed. Targets must stay under .stemcode/skills, .stemcode/commands, or .stemcode/memory.");
        }
        ...
        {
            throw new InvalidOperationException(
                $"Skill target '{target}' is not allowed.");
        }
        ...
        {
            throw new InvalidOperationException(
                $"Skill target '{target}' escapes the workspace.");
        }
    ...
    private static string NormalizeRelativePath(string path)
    {
        string normalized = NormalizeOptional(path)
            ?? throw new InvalidOperationException("Skill file paths cannot be empty.");
        ...
        {
            throw new InvalidOperationException(
                $"Skill path '{path}' must be relative.");
        }
        ...
        {
            throw new InvalidOperationException(
                $"Skill path '{path}' is not allowed.");
        }
