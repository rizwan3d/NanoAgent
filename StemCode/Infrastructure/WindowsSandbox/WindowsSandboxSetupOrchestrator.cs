...
{
    public const string SetupCommandArgument = "--stemcode-windows-sandbox-setup";
    private const string CliAssemblyFileName = "StemCode.CLI.dll";
    private const string CliExecutableFileName = "StemCode.CLI.exe";
    ...
    internal static bool IsSetupFresh(WindowsSandboxSetupPayload payload)
    {
        ...
        WindowsSandboxPaths.EnsureStateDirectories(payload.StemCodeHome);
        string markerPath = WindowsSandboxPaths.SetupMarkerPath(payload.StemCodeHome);
        string usersPath = WindowsSandboxPaths.SandboxUsersPath(payload.StemCodeHome);
    ...
    public static int RunEncodedSetupPayload(string payloadB64)
    {
        ...
        try
        {
            ...
            ClearSetupErrorReport(payload.StemCodeHome);
            return 0;
        }
        catch (Exception exception)
        {
            WriteSetupErrorReport(payload.StemCodeHome, exception);
            return 1;
        }
    }
    ...
    public static void RunSetup(WindowsSandboxSetupPayload payload, bool needsElevation)
    {
        ...
        try
        {
            ...
            if (exitCode != 0)
            {
                throw ReadSetupErrorOrExitCode(payload.StemCodeHome, unchecked((int)exitCode));
            }
    ...
    private static (string FileName, string ParametersPrefix) CreateSelfSetupLaunchCommand()
    {
        string processPath = Environment.ProcessPath ??
            throw new InvalidOperationException("Unable to locate the current StemCode executable for elevated Windows sandbox setup.");
        ...
        if (!File.Exists(entryAssemblyPath))
        {
            string entryAssemblyName = Assembly.GetEntryAssembly()?.GetName().Name ??
                throw new InvalidOperationException("Unable to locate the current StemCode assembly for elevated Windows sandbox setup.");
    ...
    private static void ExecuteSetup(WindowsSandboxSetupPayload payload)
    {
        ...
        {
            WindowsSandboxUserProvisioner.EnsureUsers(payload.StemCodeHome);
        }
        else
        {
            WindowsSandboxPaths.EnsureStateDirectories(payload.StemCodeHome);
        ...
        WindowsSandboxHelperMaterializer.RefreshHelperArtifacts(payload.StemCodeHome);
    ...
    {
        string stemCodeHome = payload.StemCodeHome;
    ...
    private static void ClearSetupErrorReport(string stemCodeHome)
    {
        string path = WindowsSandboxPaths.SetupErrorPath(stemCodeHome);
        if (File.Exists(path))
        {
            File.Delete(path);
        }
    ...
    private static Exception ReadSetupErrorOrExitCode(string stemCodeHome, int exitCode)
    {
        string path = WindowsSandboxPaths.SetupErrorPath(stemCodeHome);
        if (File.Exists(path))
        {
    ...
    private static void WriteSetupErrorReport(string stemCodeHome, Exception exception)
    {
        WindowsSandboxPaths.EnsureStateDirectories(stemCodeHome);
        File.WriteAllText(
            WindowsSandboxPaths.SetupErrorPath(stemCodeHome),
            JsonSerializer.Serialize(
