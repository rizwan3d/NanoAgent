...
{
    public const string RunnerCommandArgument = "--stemcode-windows-sandbox-runner";
        ...
        CancellationToken cancellationToken)
    {
        ...
        WindowsSandboxPaths.EnsureStateDirectories(context.StemCodeHome);
        WindowsSandboxLog.Write(context.StemCodeHome, $"command start: mode={context.Mode}, cwd={context.CommandCwd}, file={request.FileName}, private_desktop={context.UsePrivateDesktop}");
        ...
        WindowsSandboxLog.Write(context.StemCodeHome, $"command exit: code={result.ExitCode}");
        return result;
    }
        ...
        CancellationToken cancellationToken)
    {
        ...
        WindowsSandboxPaths.EnsureStateDirectories(context.StemCodeHome);
        WindowsSandboxLog.Write(context.StemCodeHome, $"background command start: mode={context.Mode}, cwd={context.CommandCwd}, file={request.FileName}");
        ...
        string workspaceSid = WindowsCapabilitySidStore.WorkspaceSidForCwd(context.StemCodeHome, context.CommandCwd);
        ...
        try
        {
            ...
            WindowsSandboxBackgroundProcess handle = await LaunchBackgroundViaSelfRunnerAsync(
                ...
                cancellationToken);
            transferred = true;
            WindowsSandboxLog.Write(context.StemCodeHome, "background command launched");
            return handle;
        }
        catch (Exception exception)
        {
            WindowsSandboxLog.Write(context.StemCodeHome, $"background command failure: {exception.GetType().Name}: {exception.Message}");
            throw;
        }
        finally
        {
            if (!transferred)
            {
                foreach ((string path, string sid, FileSystemRights rights, AccessControlType type) in temporaryAces)
                {
                    ...
                    {
                        WindowsSandboxLog.Write(context.StemCodeHome, $"cleanup failure: revoke ACE {path}: {exception.Message}");
                    }
                }
            }
        }
        ...
        Func<ProcessExecutionRequest, WindowsSandboxExecutionContext, CancellationToken, Task<ProcessExecutionResult>> launcher)
    {
        ...
        {
            WindowsSandboxLog.Write(context.StemCodeHome, $"command failure: {exception.GetType().Name}: {exception.Message}");
            throw;
        }
        finally
        {
        ...
        CancellationToken cancellationToken)
    {
        ...
        WindowsSandboxLaunchCommand launchCommand = CreateSelfLaunchCommand(context.StemCodeHome);
        ...
        for (int attempt = 0; attempt < 2; attempt++)
        {
            ...
            try
            {
                credentials = WindowsSandboxIdentity.LoadOfflineCredentials(context.StemCodeHome);
            }
            catch (Exception exception)
            {
                ...
                credentialDiagnostics.Log(context.StemCodeHome);
                WindowsSandboxLog.Write(context.StemCodeHome, $"runner credential load failure: {exception.GetType().Name}: {exception.Message}");
                if (attempt == 0 && TryRecoverRunnerStartup(context, 1326))
                {
                    WindowsSandboxLog.Write(context.StemCodeHome, "runner credential recovery attempted");
                    continue;
                }
            ...
            {
                WindowsSandboxLog.Write(context.StemCodeHome, $"runner startup preflight recovery attempted: error={exception.NativeErrorCode}");
                continue;
            }
            ...
            diagnostics.Log(context.StemCodeHome);
            if (attempt == 0 && TryRecoverRunnerStartup(context, error))
            {
                WindowsSandboxLog.Write(context.StemCodeHome, $"runner startup recovery attempted: error={error}");
                continue;
            }
        ...
        CancellationToken cancellationToken)
    {
        ...
        for (int attempt = 0; attempt < 2; attempt++)
        {
            ...
            {
                WindowsSandboxLog.Write(context.StemCodeHome, $"background runner credential load failure: {exception.GetType().Name}: {exception.Message}");
                if (attempt == 0 && TryRecoverRunnerStartup(context, 1326))
                {
                    continue;
                }
            ...
            {
                WindowsSandboxLog.Write(context.StemCodeHome, $"background runner startup preflight recovery attempted: error={exception.NativeErrorCode}");
                continue;
            }
            ...
            if (attempt == 0 && TryRecoverRunnerStartup(context, error))
            {
                WindowsSandboxLog.Write(context.StemCodeHome, $"background runner startup recovery attempted: error={error}");
                continue;
            }
        ...
        WindowsAllowDenyPaths paths)
    {
        WindowsSandboxSetupOrchestrator.EnsureSetupFresh(
            ...
            {
                StemCodeHome = context.StemCodeHome,
    ...
    {
        if (!File.Exists(WindowsSandboxPaths.SetupMarkerPath(context.StemCodeHome)) ||
            !File.Exists(WindowsSandboxPaths.SandboxUsersPath(context.StemCodeHome)))
        {
        ...
        EnsureSandboxBinUsable(context.StemCodeHome, startupContext);
        ...
        WindowsSandboxRunnerStartupContext startupContext)
    {
        ...
        diagnostics.Log(startupContext.StemCodeHome);
        throw new Win32Exception(error, diagnostics.Text);
    }
    ...
    private static void EnsureSandboxBinUsable(
        ...
        WindowsSandboxRunnerStartupContext startupContext)
    {
        string sandboxBin = WindowsSandboxPaths.SandboxBinDir(stemCodeHome);
        ...
        int error)
    {
        ...
        {
            WindowsSandboxLog.Write(context.StemCodeHome, $"runner startup recovery canceled: {exception.Message}");
        ...
        {
            WindowsSandboxLog.Write(context.StemCodeHome, $"runner startup recovery failed: {exception.GetType().Name}: {exception.Message}");
            return false;
        }
    }
    ...
    internal static WindowsSandboxLaunchCommand CreateSelfLaunchCommand(string stemCodeHome)
        => WindowsSandboxHelperMaterializer.ResolveSelfLaunchCommand(stemCodeHome);
    ...
    {
        return BuildEnvironmentBlockBytes(environmentVariables, stemCodeHome: null);
    }
        ...
        string? stemCodeHome)
    {
        ...
        if (!string.IsNullOrWhiteSpace(stemCodeHome))
        {
            ApplySandboxRuntimeEnvironment(merged, stemCodeHome);
        }
        ...
        CancellationToken cancellationToken)
    {
        ...
        for (int attempt = 0; attempt < retryDelays.Length; attempt++)
        {
            ...
            if (attempt > 0)
            {
                EnsureSandboxRuntimeLayout(stemCodeHome);
                WindowsSandboxLog.Write(
                    stemCodeHome,
    ...
    internal static WindowsSandboxRuntimeLayout EnsureSandboxRuntimeLayout(string stemCodeHome)
    {
        WindowsSandboxPaths.EnsureStateDirectories(stemCodeHome);

        string runtimeDir = WindowsSandboxPaths.SandboxRuntimeDir(stemCodeHome);
        string profileDir = WindowsSandboxPaths.SandboxRuntimeProfileDir(stemCodeHome);
        string tempDir = WindowsSandboxPaths.SandboxRuntimeTempDir(stemCodeHome);
        ...
        WindowsSandboxExecutionContext context)
    {
        ...
        if (IsUnderPath(executablePath, windowsDirectory) ||
            IsUnderPath(executablePath, context.StemCodeHome))
        {
            return request;
        }
        ...
        CancellationToken cancellationToken)
    {
        ...
        PrepareRuntimeProfileAcls(context.StemCodeHome, capabilitySids, sandboxGroupSid, temporaryAces);
        ...
        if (ShouldGrantPlatformRootAcls())
        {
            foreach (string platformRoot in WindowsSandboxSetupRoots.PlatformDefaultReadRoots)
            {
                ...
                {
                    WindowsSandboxLog.Write(context.StemCodeHome,
    ...
    {
        WindowsSandboxRuntimeLayout layout = EnsureSandboxRuntimeLayout(stemCodeHome);
        string sandboxDir = WindowsSandboxPaths.SandboxDir(stemCodeHome);
        string sandboxSecretsDir = WindowsSandboxPaths.SandboxSecretsDir(stemCodeHome);

        (string Path, FileSystemRights Rights)[] paths =
        [
            (stemCodeHome, FileSystemRights.ReadAndExecute | FileSystemRights.ListDirectory),
        ...
        [
            WindowsSandboxPaths.CapSidFile(stemCodeHome),
            WindowsSandboxPaths.SetupMarkerPath(stemCodeHome),
            WindowsSandboxPaths.SandboxUsersPath(stemCodeHome)
        ];
        ...
        CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            byte[] environmentBytes = BuildEnvironmentBlockBytes(request.EnvironmentVariables, context.StemCodeHome);
            ...
            WindowsSandboxLog.Write(context.StemCodeHome, $"restricted child launch: desktop={desktop.DesktopName}");
        ...
        string? stemCodeHome = null)
    {
        ...
        if (created)
        {
            if (!string.IsNullOrWhiteSpace(stemCodeHome))
            {
                WindowsSandboxLog.Write(stemCodeHome, "restricted child launch path: CreateProcessAsUser");
            }
        ...
        try
        {
            if (!string.IsNullOrWhiteSpace(stemCodeHome))
            {
                WindowsSandboxLog.Write(stemCodeHome, $"restricted child launch path: CreateProcessWithTokenW fallback after {createProcessAsUserError}");
            }
        ...
        CancellationToken cancellationToken)
    {
        ...
        WindowsSandboxLog.Write(context.StemCodeHome, "background child launch path: sandbox-user-direct (streaming)");
