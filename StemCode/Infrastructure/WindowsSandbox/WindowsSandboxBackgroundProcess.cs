...
internal sealed class WindowsSandboxBackgroundProcess : IBackgroundProcessHandle
{
    ...
    private async Task KillCoreAsync()
    {
        ...
        {
            WindowsSandboxLog.Write(_stemCodeHome, $"background terminate send failed: {exception.GetType().Name}: {exception.Message}");
        }
        ...
        {
            WindowsSandboxLog.Write(_stemCodeHome, "background terminate grace expired; force-terminating runner");
        }
    ...
    private void RevokeTemporaryAces()
    {
        foreach ((string path, string sid, FileSystemRights rights, AccessControlType type) in _temporaryAces)
        {
            ...
            {
                WindowsSandboxLog.Write(_stemCodeHome, $"background cleanup failure: revoke ACE {path}: {exception.Message}");
            }
        }
    }
