...
internal static class WindowsSandboxHelperMaterializer
{
    ...
    public static WindowsSandboxLaunchCommand ResolveSelfLaunchCommand(string stemCodeHome)
    {
        string processPath = Environment.ProcessPath ??
            throw new InvalidOperationException("Unable to locate the current StemCode executable for Windows sandbox runner.");
    ...
    public static void RefreshHelperArtifacts(string stemCodeHome)
    {
        string processPath = Environment.ProcessPath ??
            throw new InvalidOperationException("Unable to locate the current StemCode executable for Windows sandbox runner.");
        string sourceDirectory = AppContext.BaseDirectory;
    ...
    public static string MaterializeExternalExecutable(string stemCodeHome, string sourceExecutablePath)
    {
        ArgumentException.ThrowIfNullOrWhiteSpace(stemCodeHome);
        ...
        string bundleDirectory = Path.Combine(
            WindowsSandboxPaths.SandboxRuntimeDir(stemCodeHome),
            "tool-cache",
        ...
        IEnumerable<string?> additionalDirectories)
    {
        ...
        string[] additionalDirectoryList = additionalDirectories
            ...
            .ToArray();
        string bundleDirectory = Path.Combine(
            WindowsSandboxPaths.SandboxBinDir(stemCodeHome),
    ...
    private static WindowsSandboxResolvedLaunchTarget ResolveLaunchTarget(string sourceDirectory, string processPath)
    {
        ...
        throw new InvalidOperationException("Unable to locate the current StemCode assembly for Windows sandbox runner.");
    }
