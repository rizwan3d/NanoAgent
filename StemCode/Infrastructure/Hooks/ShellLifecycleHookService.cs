...
internal sealed class ShellLifecycleHookService : ILifecycleHookService
{
        ...
        string workspaceRoot)
    {
        ...
        {
            ["STEMCODE_HOOK_EVENT"] = context.EventName,
            ["STEMCODE_WORKSPACE_ROOT"] = workspaceRoot
        };

        AddOptional(environment, "STEMCODE_APPLICATION", context.ApplicationName);
        AddOptional(environment, "STEMCODE_SESSION_ID", context.SessionId);
        AddOptional(environment, "STEMCODE_TOOL_CALL_ID", context.ToolCallId);
        AddOptional(environment, "STEMCODE_TOOL_NAME", context.ToolName);
        AddOptional(environment, "STEMCODE_PATH", context.Path);
        AddOptional(environment, "STEMCODE_SHELL_COMMAND", context.ShellCommand);
        AddOptional(environment, "STEMCODE_RESULT_STATUS", context.ResultStatus);

        if (context.ShellExitCode is not null)
        {
            environment["STEMCODE_SHELL_EXIT_CODE"] = context.ShellExitCode.Value.ToString();
        }
