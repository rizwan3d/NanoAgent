...
internal sealed class StemCodeEnterpriseCredentialService :
    IStemCodeEnterpriseAuthenticator,
    IStemCodeEnterpriseCredentialService
{
    private const string CallbackPath = "/callback";
    private const string ClientId = "stemcode";
    private const string CredentialType = "stemcode-enterprise";
    ...
    private readonly ILogger<StemCodeEnterpriseCredentialService> _logger;
    private readonly Func<string, bool> _browserOpener;

    public StemCodeEnterpriseCredentialService(
        HttpClient httpClient,
        IAgentConfigurationStore configurationStore,
        IApiKeySecretStore secretStore,
        IStatusMessageWriter statusMessageWriter,
        ILogger<StemCodeEnterpriseCredentialService> logger,
    ...
    public bool CanResolve(string storedCredentials)
    {
        ...
        try
        {
            StemCodeEnterpriseCredentials? credentials = JsonSerializer.Deserialize(
                storedCredentials,
                StemCodeEnterpriseJsonContext.Default.StemCodeEnterpriseCredentials);
        ...
        CancellationToken cancellationToken)
    {
        ...
        {
            _logger.LogInformation("Unable to open the system browser for StemCode Enterprise sign-in.");
        }

        StemCodeEnterpriseAuthorizationCallback callback = await WaitForCallbackAsync(
            listener,
            state,
            timeoutSource.Token);
        StemCodeEnterpriseCredentials credentials = callback switch
        {
            ...
            { Code: not null } => await ExchangeCodeForCredentialsAsync(
                ...
                codeVerifier,
                cancellationToken),
            _ => throw new InvalidOperationException("StemCode Enterprise authentication callback was incomplete.")
        };
    ...
    public async Task<StemCodeEnterpriseResolvedCredential> ResolveAsync(
        ...
        CancellationToken cancellationToken)
    {
        ...
        StemCodeEnterpriseCredentials credentials = DeserializeCredentials(credentialsJson);
        ...
        return new StemCodeEnterpriseResolvedCredential(credentials.AccessToken);
    }
    ...
    private async Task<StemCodeEnterpriseAuthorizationCallback> WaitForCallbackAsync(
        ...
        CancellationToken cancellationToken)
    {
        while (true)
        {
            ...
            StemCodeEnterpriseAuthorizationCallback? callback = await TryHandleCallbackClientAsync(
                client,
                expectedState,
                cancellationToken);
    ...
    private static async Task<StemCodeEnterpriseAuthorizationCallback?> TryHandleCallbackClientAsync(
        TcpClient client,
        string expectedState,
        CancellationToken cancellationToken)
    {
        ...
        if (TryReadBearerTokenCallback(
                ...
                headers,
                requestBody,
                out StemCodeEnterpriseAuthorizationCallback? bearerCallback))
        {
            await WriteHttpResponseAsync(
                ...
                <p>You can close this window and return to StemCode.</p>
                </body>
                </html>
                """,
                cancellationToken);
        ...
        if (NormalizeOrNull(query.GetValueOrDefault("error")) is string error)
        {
            await WriteHttpResponseAsync(
                ...
                cancellationToken);
            throw new InvalidOperationException($"StemCode Enterprise authentication failed: {error}");
        }
        ...
        if (code is null || state is null)
        {
            ...
            throw new InvalidOperationException("StemCode Enterprise authentication returned an incomplete callback.");
        }

        if (!string.Equals(state, expectedState, StringComparison.Ordinal))
        {
            await WriteHttpResponseAsync(stream, HttpStatusCode.BadRequest, "State mismatch.", cancellationToken);
            throw new InvalidOperationException("StemCode Enterprise authentication state did not match.");
        }
        ...
        return StemCodeEnterpriseAuthorizationCallback.FromCode(code);
    }

    private async Task<StemCodeEnterpriseCredentials> ExchangeCodeForCredentialsAsync(
        string providerBaseUrl,
        string code,
        string redirectUri,
        string codeVerifier,
        CancellationToken cancellationToken)
    {
        ...
        if (!response.IsSuccessStatusCode)
        {
            throw new InvalidOperationException(
                $"StemCode Enterprise token exchange failed with HTTP {(int)response.StatusCode}.");
        }

        StemCodeEnterpriseTokenResponse payload = JsonSerializer.Deserialize(
            responseBody,
            StemCodeEnterpriseJsonContext.Default.StemCodeEnterpriseTokenResponse) ??
            throw new InvalidOperationException("StemCode Enterprise token exchange returned an invalid response.");

        if (string.IsNullOrWhiteSpace(payload.AccessToken) || string.IsNullOrWhiteSpace(payload.RefreshToken))
        {
            throw new InvalidOperationException("StemCode Enterprise token exchange returned incomplete credentials.");
        }

        return new StemCodeEnterpriseCredentials(
            CredentialType,
            providerBaseUrl,
            payload.AccessToken,
    ...
    private async Task<StemCodeEnterpriseCredentials> RefreshCredentialsAsync(
        StemCodeEnterpriseCredentials credentials,
        CancellationToken cancellationToken)
    {
        if (string.IsNullOrWhiteSpace(credentials.RefreshToken))
        {
            throw new InvalidOperationException("StemCode Enterprise credentials do not include a refresh token.");
        }
        ...
        {
            throw new InvalidOperationException(
                $"StemCode Enterprise token refresh failed with HTTP {(int)response.StatusCode}.");
        }

        StemCodeEnterpriseTokenResponse payload = JsonSerializer.Deserialize(
            responseBody,
            StemCodeEnterpriseJsonContext.Default.StemCodeEnterpriseTokenResponse) ??
            throw new InvalidOperationException("StemCode Enterprise token refresh returned an invalid response.");

        if (string.IsNullOrWhiteSpace(payload.AccessToken) || string.IsNullOrWhiteSpace(payload.RefreshToken))
        {
            throw new InvalidOperationException("StemCode Enterprise token refresh returned incomplete credentials.");
        }
    ...
    private static StemCodeEnterpriseCredentials CreateAccessTokenCredentials(
        string providerBaseUrl,
        string accessToken,
    ...
    private static bool IsExpired(StemCodeEnterpriseCredentials credentials)
    {
    ...
    private static string SerializeCredentials(StemCodeEnterpriseCredentials credentials)
    {
    ...
    private static StemCodeEnterpriseCredentials DeserializeCredentials(string storedCredentials)
    {
        StemCodeEnterpriseCredentials? credentials = JsonSerializer.Deserialize(
            storedCredentials,
            StemCodeEnterpriseJsonContext.Default.StemCodeEnterpriseCredentials);
        ...
        {
            throw new InvalidOperationException("StemCode Enterprise credentials are invalid.");
        }
    ...
    private static bool TryReadBearerTokenCallback(
        ...
        IReadOnlyDictionary<string, string> headers,
        string requestBody,
        out StemCodeEnterpriseAuthorizationCallback? callback)
    {
        callback = null;
        ...
        long? expiresAtUnixTimeMilliseconds = ParseExpiryHeader(headers.GetValueOrDefault("X-StemCode-Token-Expires-At")) ??
            ParseExpiryBody(requestBody);

        callback = StemCodeEnterpriseAuthorizationCallback.FromAccessToken(
    ...
    private sealed record StemCodeEnterpriseAuthorizationCallback(
        string? Code,
        string? AccessToken,
        long? ExpiresAtUnixTimeMilliseconds)
    {
        public static StemCodeEnterpriseAuthorizationCallback FromCode(string code)
        {
            return new(code, AccessToken: null, ExpiresAtUnixTimeMilliseconds: null);
        }

        public static StemCodeEnterpriseAuthorizationCallback FromAccessToken(
            string accessToken,
            long? expiresAtUnixTimeMilliseconds)
        {
