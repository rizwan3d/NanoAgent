...
### C# (StemCode, StemCode.CLI, StemCode.Desktop, StemCode.Tests)
...
|---|---|---|
| Namespaces | `PascalCase`, project-rooted | `StemCode.Application.Backend`, `StemCode.CLI.Presentation` |
| Interfaces | `I` prefix, `PascalCase` | `IStemCodeBackend`, `IConversationPipeline` |
| Classes/Structs | `PascalCase` | `StemCodeBackend`, `AgentConversationPipeline` |
...
- Typed exception hierarchy in `StemCode.Application.Exceptions`
...
- GitLab CI config at `.gitlab-ci.yml`, Bitbucket script at `.bitbucket/stemcode-bitbucket-review.sh`
- `.stemcode/.stemcodeignore` manages which files/patterns are excluded from agent workspace scanning
