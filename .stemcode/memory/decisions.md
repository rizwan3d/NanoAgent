...
**Context**: StemCode is distributed as a single-file, self-contained binary across Windows, Linux, and macOS. AOT provides fast startup, small footprint, and no runtime JIT dependency.  
**Consequences**:
...
**Date**: Project inception  
**Decision**: IDE plugins (JetBrains, VS Code) communicate with the StemCode CLI via a JSON-RPC 2.0 protocol called ACP over stdin/stdout.  
...
### 4. Hosted Service Pattern via StemCodeHostFactory
**Date**: Project inception  
**Decision**: `StemCodeHostFactory.Create()` builds an `IHost` using `Host.CreateEmptyApplicationBuilder()` with manual service registration, rather than `Host.CreateDefaultBuilder()`.  
