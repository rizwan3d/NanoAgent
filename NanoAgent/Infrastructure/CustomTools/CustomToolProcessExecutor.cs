...
internal sealed class CustomToolProcessExecutor
{
        ...
        string sandboxEnforcement)
    {
        ...
        environment["NANOAGENT_TOOL_NAME"] = context.ToolName;
        environment["NANOAGENT_CUSTOM_TOOL_NAME"] = configuration.Name;
        environment["NANOAGENT_SESSION_ID"] = context.Session.SessionId;
        environment["NANOAGENT_WORKSPACE_PATH"] = context.Session.WorkspacePath;
        environment["NANOAGENT_WORKING_DIRECTORY"] = context.Session.WorkingDirectory;
        environment["NANOAGENT_SANDBOX_MODE"] = ToWireValue(_permissionSettings.SandboxMode);
        environment["NANOAGENT_SANDBOX_EFFECTIVE_MODE"] = ToWireValue(_permissionSettings.SandboxMode);
        environment["NANOAGENT_SANDBOX_ENFORCEMENT"] = sandboxEnforcement;
        environment["NANOAGENT_SANDBOX_PERMISSIONS"] = ShellCommandSandboxArguments.ToWireValue(
            ShellCommandSandboxPermissions.UseDefault);
        environment["NANOAGENT_WORKSPACE_ROOT"] = workspaceRoot;
        environment["NANOAGENT_CUSTOM_TOOL_SOURCE"] = configuration.SourcePath ?? string.Empty;
        environment["NANOAGENT_CUSTOM_TOOL_TRUSTED"] = configuration.UntrustedWorkspaceDefinition ? "0" : "1";
        environment["NANOAGENT_CUSTOM_TOOL_WORKING_DIRECTORY"] = workingDirectory;
